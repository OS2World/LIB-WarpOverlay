...
#include "i81xregs.h"
#include "drv_priv.h"
#define pRegs ((PI81XREGS)HWData)

ULONG I81X_VideoCaps(PHWVIDEOCAPS pCaps) {
...
ULONG  I81X_SetVideoAttr(PHWATTRIBUTE pAttr,ULONG AttrNum) {
      ...
      switch (AttrNum) {
        ...
        case 2: //contrast
             pRegs->OV0CLRC0 &= 0xff;
             pRegs->OV0CLRC0 |= (pAttr->ulCurrentValue&0xff) << 9;
             return RC_SUCCESS;
        ...
        case 4: //Color Keying
             pRegs->chipflags &= ~I81X_DIS_COLORKEY;
             if (!pAttr->ulCurrentValue) pRegs->chipflags |= I81X_DIS_COLORKEY;
             return RC_SUCCESS;
...
ULONG  I81X_GetVideoAttr(PHWATTRIBUTE pAttr,ULONG AttrNum) {
      ...
      switch (AttrNum) {
        ...
        case 2:  //contrast
             ...
             pAttr->ulAttribFlags=0;
             pAttr->ulCurrentValue=(pRegs->OV0CLRC0>>9)&0xff;
        ...
        case 4: //Color Keying
             ...
             pAttr->ulAttribFlags=0;
             pAttr->ulCurrentValue=pRegs->chipflags&I81X_DIS_COLORKEY ? FALSE:TRUE;
...
ULONG  I81X_SetupVideo(PHWVIDEOSETUP pSetup) {
ULONG dst_w,dst_h,src_w,src_h;
    ...
    if (pRegs->chipflags&I81X_DIS_COLORKEY) pRegs->DCLRKM=0;
    else
    ...
    pRegs->SCLRKM=0;
    pRegs->OV0CONF=1;
    pRegs->reserved=0xdeadbeaf;
    return RC_SUCCESS;
}

ULONG I81X_DisplayVideo(ULONG BufferOffset){
    pRegs->OBUF_0Y=BufferOffset;
    pRegs->OBUF_1Y=BufferOffset;
    pRegs->OV0CMD^=4;
    memcpy(pRegs->RegsPtr,pRegs,sizeof(I81XREGS));
    WRITEREG(MMIO,0x30000,pRegs->RegsPhys|0x80000000);
    return RC_SUCCESS;
}

ULONG I81X_HideVideo(void) {
      pRegs->OV0CMD=0;
      return I81X_DisplayVideo(0);
}


ULONG I81X_RestoreVideo(void) {
    I81X_DisplayVideo(pRegs->OBUF_0Y);
    return RC_SUCCESS;
}
...
ULONG rc,temp,temp1;
    ...
    if ((temp==0x7121)||(temp==0x7123)||(temp==0x7125)||(temp==0x1132)) {
       pci_read_dword(&PciDevice,0x14,&temp1);
       temp1&=0xffff0000;
       MMIO=PhysToLin(temp1,0x31000);
       if (!MMIO) return RC_ERROR;
       pRegs->OV0CLRC0=0x4000;
       pRegs->OV0CLRC1=0x80;
       HideVideo=I81X_HideVideo;
       VideoCaps=I81X_VideoCaps;
       SetVideoAttr=I81X_SetVideoAttr;
       GetVideoAttr=I81X_GetVideoAttr;
       DisplayVideo=I81X_DisplayVideo;
       SetupVideo=I81X_SetupVideo;
       RestoreVideo=I81X_RestoreVideo;
       /* Get GART physical address */
       temp1=(READREG(MMIO,0x2020))&0xfffff000;
       /* Map GART to linear space */
       pRegs->Gart=PhysToLin(temp1,65536);
       if (!pRegs->Gart) return RC_ERROR;
       /* Calculate VRAM size */
       temp1=0;
       while ((temp1<65536)&&(READREG(pRegs->Gart,temp1)&1)) temp1+=4;
       pRegs->RegsPhys=READREG(pRegs->Gart,temp1-4)&0x3ffff000;
       pRegs->RegsPtr=PhysToLin(pRegs->RegsPhys,4096);
